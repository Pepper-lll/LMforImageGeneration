...
IMAGE_EXTENSIONS = {"png", "bmp", "jpg", "jpeg", "pgm", "ppm", "tif", "tiff", "webp"}
...
def main():
    ...
    print("@ sample from:", args.sample_batch)
    ...
    def read_activations(self, npz_path: str) -> Tuple[np.ndarray, np.ndarray]:
        if npz_path.endswith(".npz"):
            ...
            for i in tqdm(range(0, len(files))):
                ...
                samples.append(np.asarray(image).astype(np.uint8))
            samples = np.stack(samples)
            # breakpoint()
            iter_nums = len(samples) // self.batch_size
            iter_samples = samples[: iter_nums * self.batch_size].reshape(
                [iter_nums, self.batch_size, *samples.shape[1:]]
            )
            # breakpoint()
            return self.compute_activations(iter(iter_samples))
